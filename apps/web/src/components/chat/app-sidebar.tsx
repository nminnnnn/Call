...
export function AppSidebar({ currentUser, onLogout }: { currentUser?: Person; onLogout: () => void }) {
  return (
    <nav className="app-nav" aria-label="Điều hướng ứng dụng">
      ...
      ))}<Tooltip label="Đăng xuất"><button className="app-nav__item" type="button" onClick={onLogout}><LogOut size={21} /><span>Đăng xuất</span></button></Tooltip></div>
