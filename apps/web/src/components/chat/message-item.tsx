...
import { Check, CheckCheck, CornerUpLeft, Loader2, Pencil, SmilePlus, Trash2, X } from "lucide-react";
...
export function MessageItem({ message, author, currentUserId, replyMessage, replyAuthor, attachments, onReply, onReaction, onEdit, onDelete }: { message: Message; author?: Person; currentUserId: string; replyMessage?: Message; replyAuthor?: Person; attachments: Attachment[]; onReply: (message: Message) => void; onReaction: (messageId: string, emoji: string) => void; onEdit: (messageId: string, body: string) => Promise<void>; onDelete: (messageId: string) => Promise<void> }) {
  ...
  return (
    <article className={`message ${mine ? "message--mine" : ""}`}>
      ...
      <div className="message__content">
        ...
        <div className="message__meta"><time>{formatTime(message.createdAt)}</time>{message.editedAt && <span>đã sửa</span>}{mine && <DeliveryIcon status={message.status} />}</div>
