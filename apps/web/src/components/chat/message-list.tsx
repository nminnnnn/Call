...
export function MessageList({ conversation, messages, attachments, people, currentUserId, loading, typingUsers, onReply, onReaction, onEdit, onDelete }: { conversation: ConversationSummary; messages: Message[]; attachments: Map<string, Attachment>; people: Map<string, Person>; currentUserId: string; loading: boolean; typingUsers: Person[]; onReply: (message: Message) => void; onReaction: (messageId: string, emoji: string) => void; onEdit: (messageId: string, body: string) => Promise<void>; onDelete: (messageId: string) => Promise<void> }) {
  ...
  return (
    <div className="message-feed">
      {messages.map((message, index) => {
        ...
        return <div key={message.id}>{showDate && <DateDivider label={formatDateDivider(message.createdAt)} />}{index === unreadStart && <UnreadDivider ref={unreadRef} count={conversation.unreadCount} />}<MessageItem message={message} author={people.get(message.authorId)} currentUserId={currentUserId} replyMessage={reply} replyAuthor={reply ? people.get(reply.authorId) : undefined} attachments={messageAttachments} onReply={onReply} onReaction={onReaction} onEdit={onEdit} onDelete={onDelete} /></div>;
      })}
